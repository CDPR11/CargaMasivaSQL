...
const selectTipoSQL = document.getElementById("tipoSQL")
let nombreDocumento = ''
let tipoSentencia = ''
let sentencia = ''
let tableName = ''
...
selectTipoSQL.addEventListener('change', function (){
	
	switch(document.getElementById('tipoSQL').value)
    {
		case '1' : 
...
function crearInsert (param){
    ...
    for(let i = 0; i < excel.rows().length; i++){
        ...
        for( let j = 0; j < excel.rows()[i].length; j++){
            ...
            /*if(j === excel.rows()[j].length - 1 )
                ...
                contentRow = `${contentRow} ${excel.rows()[i][j].toString().toLowerCase().includes('null') || excel.rows()[i][j].toString().toLowerCase().includes('getdate()') ?excel.rows()[i][j]:`'${excel.rows()[i][j]}'`},`
            */
			const cellValue = excel.rows()[i][j];
			const formattedCellValue = cellValue.toString().toLowerCase().includes('null') || cellValue.toString().toLowerCase().includes('getdate()')
				? cellValue
				: `'${cellValue}'`;

			contentRow += (j === excel.rows()[j].length - 1) ? formattedCellValue : `${formattedCellValue},`;
    ...
    estructura = estructura + finalCatch
    contentXls.value = sqlFormatter.format(estructura)
}

function crearDelete (param){
	let excel = param
    let arrayChecks = [] 
	let contentRow = ''
	
    for(let i = 0; i < excel.header().length; i++){
		contentRow = `${contentRow} deleted.${excel.header()[i]} AS ${excel.header()[i]}_old${i != (excel.header().length - 1) ? ',':''}`
        arrayChecks.push(excel.header()[i])
    }
    ...
    for(let i = 0; i < excel.rows().length; i++){
		...
		sentencia = `DELETE FROM [dbo].[${tableName}]
							 OUTPUT ${contentRow}
							 WHERE ${whereString}
							 PRINT('Se eliminó con exito el registro ${whereString.replaceAll("'","")}')`
		
        estructura = `${estructura}\n       IF EXISTS ( SELECT 1 FROM ${tableInput.value} WHERE ${whereString})
        BEGIN
            ${sentencia}
        END
        ELSE
        BEGIN
            PRINT('El registro con ${whereString.replaceAll("'","")} no existe');
        END`
